...
- ทุกตารางที่ผูกกับร้าน (products, orders, stock_movements, stock_plans, daily_closures, cash_days) มี `store_id` โดย `daily_closures` และ `cash_days` ใช้ primary key ร่วม (`store_id`, `report_date`) ส่วน sku และเลขออเดอร์ unique ภายในร้านของตัวเอง — ปัจจุบันมีร้านเดียว (`promttak`) และทุกอย่างยังทำงานเหมือนเดิม

...
interface CartTotalsProps { totals: Totals; discountState: DiscountState; totalQuantity: number; paidQuantity: number; rules: PricingRules; onDiscountChange(value: string): void; }

export function CartTotals({ totals, discountState, totalQuantity, paidQuantity, rules, onDiscountChange }: CartTotalsProps) {
  ...
  return <section className="cart-totals" aria-label="ยอดรวมตะกร้า">
    ...
    {totals.bundleSets > 0 && !discountState.manual && rules.bundle && <p className="promotion-note" role="status">โปรฯ {rules.bundle.unitPrice} บาท ครบ {rules.bundle.quantity} ชิ้น ลดให้อัตโนมัติ {formatCurrency(totals.autoDiscount)}</p>}
    {totals.storeDiscount > 0 && !discountState.manual && rules.wholesale && <p className="promotion-note" role="status">ราคาร้านค้า · {rules.wholesale.category} ลดชิ้นละ {rules.wholesale.discountPerItem} บาท รวม {formatCurrency(totals.storeDiscount)}</p>}

...
export function SellPage() {
  ...
  const updateCart = (next: CartItem[]) => { setCart(next); setStockNotice(''); };
  const clearCart = () => { setCart([]); setDiscountState(resetManualDiscount()); setStockNotice(''); };
  ...
  return <section className="data-page sell-page">
    ...
    <div className="sell-workspace">
      ...
      <Cart products={products} cart={cart} totals={totals} discountState={discountState} totalQuantity={totalQuantity} paidQuantity={paidQuantity} customerType={customerType} paymentMethod={paymentMethod} mobile={mobile} open={cartOpen} onClose={() => setCartOpen(false)} onClear={clearCart} onQuantityChange={(product: CatalogProduct, delta: number) => updateCart(changeQuantity(cart, product, delta))} onGiveawayChange={(productId, delta) => updateCart(changeGiveawayQuantity(cart, productId, delta))} onRemove={(productId) => updateCart(removeFromCart(cart, productId))} onDiscountChange={(value) => setDiscountState(setManualDiscount(Number.isNaN(value) ? 0 : value))} onCustomerChange={setCustomerType} onPaymentChange={setPaymentMethod} />
    </div>

...
> ทั้งสองแอปเลือกร้านได้และคิดราคาตามกฎของร้านที่เลือก โดยขอกฎจาก `GET /api/pricing-rules` ไม่ได้ฝังไว้ในโค้ด — ร้านที่เพิ่มใหม่จึงไม่รับโปรโมชั่นของร้านอื่นมาเอง
>
> ฟีเจอร์ที่ยังมีเฉพาะบน `/next/` คือ **การขายออฟไลน์และการ Sync** กับ **เงินทอนตั้งต้นประจำวัน**

## การจัดการร้าน

ระบบรองรับหลายร้านในชุด deploy เดียว โดยใช้ PIN เดิมแล้วเลือกร้านหลังเข้าสู่ระบบ ถ้ามีร้านเดียวที่เปิดใช้งาน ระบบเลือกให้อัตโนมัติและไม่แสดงหน้าเลือกร้าน
...
ร้านที่สร้างใหม่**ไม่มีโปรโมชั่นอัตโนมัติ** จนกว่าจะตั้งด้วย `pricing` — ใช้ส่วนลดมือได้ตามปกติระหว่างนั้น กฎราคาผูกกับร้าน ร้านหนึ่งจึงไม่รับโปรฯ ของอีกร้านมาเอง

เมนูของร้านใหม่เพิ่มผ่านหน้าจัดการสินค้าในแอปได้เลย หลังเข้าสู่ระบบแล้วเลือกร้านนั้น

> ⚠️ **วินาทีที่มีร้านที่สองเปิดใช้งาน พนักงานจะเริ่มถูกถามว่าจะขายร้านไหนทุกครั้งที่เข้าระบบ** ทั้งที่ `/` และ `/next/` ควรรันนอกเวลาขายและแจ้งพนักงานก่อน

ไม่มีคำสั่งลบร้าน เพราะออเดอร์ สต็อก และการปิดยอดผูกกับร้านไว้ถาวร ถ้าเลิกใช้ให้ปิดการใช้งาน (`is_active = 0`) แทน

## รูปเมนู

กรอบรูปในหน้าขายเป็น **สี่เหลี่ยมจัตุรัส 1:1** ตายตัว รูปถูกย่อในเบราว์เซอร์ให้ด้านยาวไม่เกิน 800px ก่อนส่ง (`LONGEST_SIDE` ใน `frontend/src/domain/productImage.ts`) เพดานไฟล์ 400KB

**สเปกรูปที่ควรเตรียม: 1200 × 1200 px (จัตุรัส)** หรือใหญ่กว่าในอัตราส่วนเดียวกัน

ระบบจะย่อให้เหลือ 800 × 800 เอง รูปที่ครอปเป็นจัตุรัสมาแล้วจะเต็มกรอบพอดี ไม่มีแถบข้างและไม่มีส่วนไหนถูกตัด

เลือกจัตุรัสเพราะเป็นการครอปที่คนทำเองได้ด้วยตาในแอปรูปทั่วไป ซึ่งเป็นสิ่งที่ร้านต้องทำจริง

ถ้ารูปไม่ใช่จัตุรัสจะไม่ถูกครอป แต่จะมีแถบสีพื้นของการ์ดคั่นด้านที่เหลือ — เลือกแบบนี้เพราะการครอปรูปเค้กทรงสูงจะเหลือแค่ท่อนกลาง ซึ่งไม่ใช่สิ่งที่พนักงานกวาดตาหา

ขนาดจริงที่วัดได้ (css px):

| จอ | คอลัมน์ | กรอบรูป | การ์ดทั้งใบ |
|---|---:|---|---|
| เดสก์ท็อป 1440 | 3 | 203 × 203 | 205 × 333 |
| แท็บเล็ตแนวนอน 1199 | 3 | 220 × 220 | 222 × 347 |
| แท็บเล็ตแนวนอน 1180 | 3 | 213 × 213 | 215 × 341 |
| แท็บเล็ตแนวนอน 1024 | 3 | 161 × 161 | 163 × 310 |
| แท็บเล็ต 1000 | 2 | 237 × 237 | 239 × 365 |
| แท็บเล็ตแนวตั้ง 820 | 2 | 147 × 147 | 149 × 296 |
| มือถือ 390 | 2 | 175 × 175 | 177 × 293 |

ช่วง 1024–1199px ใช้ 3 คอลัมน์ เพราะเป็นขนาดเครื่องขายจริงและ 2 คอลัมน์ทำให้การ์ดสูง 455px คือเห็นเมนูแถวเดียว ต้องเลื่อนตลอด แท็บเล็ตแนวตั้งยังใช้ 2 คอลัมน์ เพราะพื้นที่แคบอยู่แล้ว

กรอบใหญ่สุด 237px บนจอ 3x ต้องการ 711 พิกเซลจริง จึงตั้งเพดานย่อไว้ที่ 800px ให้ยังเหลือที่เผื่อ

รูปในตะกร้าและหน้าจัดการเมนูเป็นสี่เหลี่ยมเล็ก ใช้การครอปเต็มกรอบ (`cover`) เพราะมีชื่อเมนูอยู่ข้างๆ อยู่แล้ว
...
| `PROMPTPAY_ID` | ไม่ | เบอร์โทรศัพท์หรือเลขประจำตัว 13 หลักสำหรับสร้าง QR พร้อมเพย์ ใช้กับร้านที่ยังไม่ได้ตั้ง QR ของตัวเอง ทั้ง Cloud Mode และ provisioning สำหรับ Local Mode |
...
### QR รับเงินของแต่ละร้าน

แต่ละร้านตั้ง QR ของตัวเองได้ที่หน้า **ตั้งค่า** ของแอป `/next/` ในส่วน "QR รับเงิน" โดยเลือกรูปจากกล้องหรือแกลเลอรี รูปจะถูกเตรียมในเบราว์เซอร์เป็น PNG ขนาดด้านยาว 1000px (ไม่ใช้การบีบอัดแบบสูญเสียถ้าไม่จำเป็น เพราะขอบของ finder pattern มีผลต่อการสแกน) แล้วเก็บเป็น bytes ในตาราง `store_payment_qr` โดยตั้งชื่อ URL ด้วย checksum

ลำดับการเลือก QR ที่ลูกค้าจะเห็น:

1. ร้านตั้งรูป QR ของตัวเองไว้ → ใช้รูปนั้น
2. ร้านยังไม่ได้ตั้ง → ใช้ `PROMPTPAY_ID` ที่ตั้งไว้ทั้งระบบ สร้าง QR ตามยอดให้อัตโนมัติ

> **รูป QR เก็บยอดชำระไม่ได้** ต่างจาก QR ที่ระบบสร้างเองซึ่งฝังยอดไว้ในตัว ร้านที่ใช้รูปของตัวเองลูกค้าจะต้องกรอกยอดเอง ทั้งสองแอปจึงแสดงยอดตัวใหญ่ข้างรูปและเตือนพนักงานให้ตรวจยอดก่อนยืนยัน

`GET /api/payment-qr` ตอบ QR ที่ถูกต้องของร้านที่ล็อกอินอยู่เสมอ และบอกชนิดผ่าน header `X-Payment-QR-Amount` (`embedded` = ยอดอยู่ใน QR, `manual` = ลูกค้ากรอกเอง) เพื่อไม่ให้เครื่องขายต้องถามก่อนหนึ่งรอบที่หน้าเคาน์เตอร์
...
ข้อมูล PromptPay ที่ provision ไว้จะผูกกับ `storeId` ของร้านที่เลือกอยู่เสมอ และ provision ใหม่ทุกครั้งที่สลับร้าน หากเครื่องถือข้อมูลของอีกร้านไว้ ระบบจะถือว่า "ยังไม่ได้เตรียมพร้อมเพย์" แทนที่จะแสดง QR ของร้านก่อนหน้า เพราะเงินที่โอนเข้าบัญชีผิดร้านจะไม่มีอะไรที่หน้าเคาน์เตอร์บอกได้จนกว่าจะปิดยอดสิ้นวัน สำหรับร้านที่ใช้รูป QR ของตัวเอง เครื่องจะเก็บ bytes ของรูปไว้ด้วย เพราะไม่มี payload ให้สร้างรูปขึ้นมาใหม่ได้

ข้อมูลผู้รับ PromptPay ที่ provision ไว้ประกอบด้วยเฉพาะ merchant-account information ที่ normalize แล้ว, เวอร์ชัน, `storeId` และเวลาที่ provision โดยเก็บใน IndexedDB store `offlinePaymentConfig` เท่านั้น ไม่เก็บใน localStorage, CacheStorage, source code หรือ public environment ข้อมูลผู้รับนี้จำเป็นต่อการสร้าง QR ออฟไลน์และถูกเข้ารหัสอยู่ใน QR ตามธรรมชาติ จึงไม่ใช่รหัสผ่านหรือ secret แต่ยังควรถือเป็นข้อมูลการชำระเงินประจำเครื่องและไม่ควร log หรือแสดงโดยไม่จำเป็น
